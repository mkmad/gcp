...
PASSWORD_FILE="./ssh_password.txt"  # Path to password file (should be git-ignored)
...
# Validate inputs
if [ -z "$TARGET_HOST" ] || [ -z "$TARGET_USER" ] || [ -z "$PASSWORD_FILE" ]; then
    echo "Error: TARGET_HOST, TARGET_USER, and PASSWORD_FILE must be set."
    exit 1
fi

# Read password from file
if [ ! -f "$PASSWORD_FILE" ] || [ ! -r "$PASSWORD_FILE" ]; then
    echo "Error: Password file '$PASSWORD_FILE' does not exist or is not readable."
    exit 1
fi
